...
- **In-game chat hub (2026-09-24, replaced the 📣 reaction list):** `GameChat.tsx` puts a plain chat button where 📣 was, top-left of the header strip, while cards are out. No unread count: every line already shows on the felt, and Jorge found a count for lines he had read there confusing. It opens a hub dropped from the TOP of the screen (so the 16px text box stays above the phone keyboard; z-[450], under the vote modals at z-[500]) with the whole game's chat (lobby + every round, divided by window), the text box, and the quick reactions as one-tap buttons that send and close. Quick buttons and typed lines are both plain chat lines (`sendChat`); `postReaction` / `room.lastReaction` are legacy (still shown for phones on an old build). New lines show on the felt through `OverlayBanner`: up to two at once, newest at the bottom, a new one fades up and pushes the older one up, a third folds the oldest away (`.felt-msg` / `.felt-msg-out` grid-rows fold in index.css), each leaves after 7 s, two text lines max. The hub is capped so its bottom edge stops at half the smallest viewport (`50svh` minus the top gap, divided by the UI zoom); the list scrolls inside it. Lines sent during a round land in that round's window, so the round-end chat box continues the conversation. The optimistic send lives in `useChatThread` (shared with the compact `Chat` box); `useChat` starts from the open connection's lines (`peekChat`), so a reader mounted mid-game never renders empty first. No server change was needed: the engine accepts chat in any status.
- **Reaction usage is counted** in the `reactionStats` collection, one doc per phrase (`src/lib/reactions.ts`, which is also the single source of truth for the hub's quick buttons: since 2026-09-24 the top five by use, ouch / why??? / take your time / respect the game / thanks!, 90% of all sends; the dropped phrases show as retired on /me). Readouts: the "Reactions used" card on `/me`, or `node scripts/reaction-stats_V01.mjs`. Localhost and games with a player named `test` are not counted. Keep the CATALOGUE in the script in sync with `REACTIONS`.
...
- **2026-09-22 evening (Jorge's list):** the dealer's trump pick renders INSIDE the felt via Table's `centerBanner` slot (never above the table: "we should always avoid pushing things down"); the undo strip is one button, "↶ Undo last move"; every `CommentaryOverlay` announcement is ONE line (no `sub`: at 1.5 s nobody read it; YOUR BID / YOUR TURN, "TRUMP IS ♥ HEARTS", WIZARD KILL!, streak and overshoot titles stand alone); whoever bids first (seat after the dealer, `src/lib/turnOrder.ts`) gets a gold LEADS pill on the bottom edge of their tile during bidding, the bid picker says "You're first: you lead the first trick" and the strip after your bid adds "you lead" (Jorge chose the pill over a status word and bid-order numerals); the replay graph is 1.2 s a round capped at 5 s total. `scripts/netlab/shot.mjs <room> <outdir> label=query…` takes phone-size PNGs of a live dev room as netA (design options go to Jorge as real screenshots).
- **Zoom-aware full height (2026-09-25, both apps):** `body { zoom: var(--ui-zoom) }` multiplies heights too, so `min-height: 100svh` made every page zoom x one screen tall (590 px of empty page under the game on an iPad). body, #root and full-screen blocks now use `calc(100svh / var(--ui-zoom, 1))`; in components use the `.min-h-screen-z` class, never Tailwind's `min-h-svh` / `min-h-screen`. Verified with `scripts/netlab/layout-check.mjs <outdir>` (plays a 1-round game vs 3 computers on the dev server and captures home, lobby, bidding, playing, round end and final at phone / big phone / iPad / desktop, plus page heights; run before and after a layout change and compare): in-game pages are now exactly one screen, layout unchanged. `scripts/netlab/sk-measure.mjs [url]` does the page-height check for the scorekeeper.
